...
impl<F: PrimeField> Circuit<F> for ManyOperationsCircuit<F> {
    ...
    fn synthesize(
        ...
        let xor_result = config.blake2b_chip.xor(
            addition_result, c, &mut layouter);
        let rotate63_result = config.blake2b_chip.rotate_right_63(
            xor_result, &mut layouter);
        let rotate16_result = config.blake2b_chip.rotate_right_16(
            rotate63_result, &mut layouter);
        let rotate24_result = config.blake2b_chip.rotate_right_24(
            rotate16_result, &mut layouter);
        let rotate32_result = config.blake2b_chip.rotate_right_32(
            rotate24_result, &mut layouter);
