...
impl<F: Field + From<u64>> SumMod64Chip<F> {
    ...
    pub fn assign_addition_rows(&mut self, layouter: &mut impl Layouter<F>, addition_trace: [[Value<F>; 6]; 3]) {
    ...
    fn assign_row_from_values(
        ...
        let _ = self.decompose_16_chip.assign_16bit_row_from_values(region, row.clone(), offset);
