...
pub fn benchmark_verification(c: &mut Criterion) {
    ...
    for amount_of_blocks in benchmarking_block_sizes() {
        ...
        benchmark_verification_iteration::<Blake2bChipOpt4Limbs<Fr>>(&params, &mut group, amount_of_blocks, "opt_4_limbs");
        benchmark_verification_iteration::<Blake2bChipOptRecycle<Fr>>(&params, &mut group, amount_of_blocks, "opt_recycle");
        benchmark_verification_iteration::<Blake2bChipOptSpread<Fr>>(&params, &mut group, amount_of_blocks, "opt_spread");
    }
    group.finish()
}
