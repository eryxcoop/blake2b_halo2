...
impl<F: PrimeField, const T: usize> LimbRotationCircuitAutogenerated<F, T> {
    pub(crate) fn new_for(input: Value<F>, expected_output: Value<F>) -> Self {
        Self {
            _ph: PhantomData,
            input,
    ...
    fn synthesize(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                let result = (&config.limb_rotation_config.limb_rotation_config).generate_rotation_rows_from_input_row(
                    &mut region,
                    &mut offset,
                    decompose_config,
                    input_row,
