...
impl<F: PrimeField> Circuit<F> for Rotation63Circuit8bitLimbsAutogenerated<F> {
    ...
    fn synthesize(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                let mut offset = 0;
                let a_row = config.rotation_63_config.decompose_8_chip.generate_row_from_value_and_keep_row(
                    &mut region,
                    self.input,
                    offset,
                )?;
                offset += 1;
                let result = config.rotation_63_config.rotation_63_chip.generate_rotation_rows_from_cells(
                    &mut region,
                    &mut offset,
