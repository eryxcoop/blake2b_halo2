...
impl AdditionMod64Config {
    ...
    fn populate_addition_rows<F: PrimeField>(
        ...
        decompose8config: Decompose8Config,
    ) -> Result<(), Error> {
        layouter.assign_region(
            ...
            |mut region| {
                ...
                decompose8config.populate_row_from_values(&mut region, &row, 0, false)?;

                let row = addition_trace[1].to_vec();
                decompose8config.populate_row_from_values(&mut region, &row, 1, false)?;

                let row = addition_trace[2].to_vec();
                decompose8config.populate_row_from_values(&mut region, &row, 2, true)?;
                Ok(())
            },
        )?;
        Ok(())
