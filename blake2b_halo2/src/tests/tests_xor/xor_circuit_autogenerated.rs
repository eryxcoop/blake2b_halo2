...
pub struct XorCircuitAutogenerated<F: PrimeField> {
    _ph: PhantomData<F>,
...
impl<F: PrimeField> XorCircuitAutogenerated<F> {
    pub fn new_for(value_a: Value<Blake2bWord>, value_b: Value<Blake2bWord>, expected_result: Value<Blake2bWord>) -> Self {
        Self {
            _ph: PhantomData,
            value_a,
            value_b,
    ...
    fn synthesize(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                let result = config.xor_config.xor_config.generate_xor_rows_from_cells(
                    &mut region,
                    &mut offset,
                    &a_cell,
                    &b_cell,
                    &config.xor_config.decompose_8_config,
                    false,
                )?.full_number
                    .clone();
