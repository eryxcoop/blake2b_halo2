...
impl<F: PrimeField> Circuit<F> for XorCircuitAutogenerated<F> {
    ...
    fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "xor_with_spread")] {
                ...
                let xor_chip = XorSpreadConfig::configure(meta, limbs_8_bits, full_number_u64, extra);
            } else if #[cfg(feature = "xor_with_table")] {
    ...
    fn synthesize(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                let mut offset = 0;
                let a_cell = config.xor_config.decompose_8_chip.generate_row_from_value(
                    &mut region,
                    self.value_a,
                    offset,
                )?;
                offset += 1;
                let b_cell = config.xor_config.decompose_8_chip.generate_row_from_value(
                    &mut region,
                    self.value_b,
                    offset,
                )?;
                ...
                let result = config.xor_config.xor_chip.generate_xor_rows_from_cells_optimized(
                    &mut region,
                    &mut offset,
                    &a_cell,
                    &b_cell,
